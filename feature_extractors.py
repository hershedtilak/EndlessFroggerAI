...
def testFeatureExtractor(state, action):
    ...
    type, dir, sinkCounter = game.getRowInfoFromPlayerCoords(y-1, board)
    if type == "RIVER":
        row = game.getRowFromPlayerCoords(y-1, board)
        relLogDir = row.getDirOfClosestLog(x)
...
def betterThanBaselineFeatureExtractor(state, action):
    ...
    for row in range(max(0, y-1), min(game.height, y+2)):
        radius = 2 - abs(row - y)
        type, dir, sinkCounter = game.getRowInfoFromPlayerCoords(row, board)
        advancedState = advancedState + game.getRowFromPlayerCoords(row, board).getValues(x, radius) + [dir, type, 1*(sinkCounter == SINK_WARN_TIME - 1)]
    
    type, dir, sinkCounter = game.getRowInfoFromPlayerCoords(y-1, board)
    if type == "RIVER":
        row = game.getRowFromPlayerCoords(y-1, board)
        relLogDir = row.getDirOfClosestLog(x)
        advancedState.append(relLogDir)
...
def safetyFeatureExtractor(state, action):
    ...
    #get cur values
    curType, curDir, curSinkCounter = game.getRowInfoFromPlayerCoords(y, board)
    ...
    # add features for moving towards logs
    type, dir, sinkCounter = game.getRowInfoFromPlayerCoords(y-1, board)
    if type == "RIVER":
        row = game.getRowFromPlayerCoords(game.player.y-1, board)
...
def moreInfoFeatureExtractor(state, action):
    ...
    for row in range(max(0, y-2), min(game.height, y+2)):
        type, dir, sinkCounter = game.getRowInfoFromPlayerCoords(row, board)
        state = tuple([row - y] + [type] + [dir] + [1*(sinkCounter == SINK_WARN_TIME - 1)] + game.getRowFromPlayerCoords(row, board).getValues(x, RADIUS) + [curDir])
